...
function a(e){
try{
for(var i=window.pageYOffset||document.documentElement.scrollTop,a=window.innerHeight||document.documentElement.clientHeight,m=0;m<S.length;m+=q){
var l=S[m];
if(!(l.offsetTop>i+a+100)){
var w=g.attr(l,"data-cpsstatus");
if("hide"==w){
l.setAttribute("data-cpsstatus","loading");
for(var u=""+m,f=1,h=m+1;h<S.length&&m+q>h;h++)u=u+"%2c"+h,f++;
var A=Math.ceil(1e7*Math.random());
if(""!==n("mockcps"))var y="/mp/cps_product_info?biz="+window.biz+"&mid="+window.mid+"&idx="+window.idx+"&cpslist="+u+"&sn="+window.sn+"&mockcps="+n("mockcps");else var y="/mp/cps_product_info?biz="+window.biz+"&mid="+window.mid+"&idx="+window.idx+"&cpslist="+u+"&sn="+window.sn+"&istempurl="+(window.is_temp_url||0)+"&random="+A;
!function(e,i,n){
_({
url:i,
type:"GET",
...
S[N].setAttribute("data-cpsstatus","hide"),S[N].setAttribute("data-showed","no");
var K={
cps_isready:!1,
cps_state:"",
pid_type:"",
...
source_logo_url:"",
is_ad:1
},M=g.attr(S[N],"data-templateid");
"list"==M?S[N].innerHTML=r.tmpl(p,K):"banner"==M?S[N].innerHTML=r.tmpl(d,K):"card"==M&&(S[N].innerHTML=r.tmpl(s,K));
}
}catch(D){
console.log(D);
}
var L,W=null;
o(),v.on(window,"scroll",o),a(),v.on(window,"scroll",E),window.is_new_msg&&-1!=navigator.userAgent.indexOf("MicroMessenger")&&(window.title&&(window.title=window.title.replace(/&#39;/g,"'").replace(/&nbsp;/g," ").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&quot;/g,'"').replace(/&amp;/g,"&")),
console.log("hd_head_img:"+hd_head_img),y.invoke("currentMpInfo",{
...
e.offsetTop<n?(t=!0,v.off(window,"scroll",o),(new Image).src=location.protocol+"//mp.weixin.qq.com/mp/jsmonitor?idkey=59977_15_1",
j.hotspotReport({
...
var P="",V=new Image;
V.src=("http://mp.weixin.qq.com/mp/jsreport?key=4&content=biz:"+biz+",mid:"+mid+",uin:"+uin+"[key4]"+P+"&r="+Math.random()).substr(0,1024);
}
...
e("appmsg/share.js");
var J=b(1e3*parseInt(top.window.modify_time)),Y=J.format("YYYY-MM-DD"),$=document.getElementById("js_modify_time");
...
for(var i=0;i<t.length;i++)"a"!=t[i].tagName.toLowerCase()&&h(t[i]);
}catch(n){}
};
try{
for(var A=document.getElementsByTagName("a"),y=0;y<A.length;y++){
var b=A.item(y),j=b.getAttribute("href")||"";
...
l&&(i>=43305||d>=16)&&44206!=i&&o.isGif()?(o=F.addParam(o,"tp","wxpic",!0),window.__addIdKeyReport("28307",91)):window.webp&&(o=F.addParam(o,"tp","webp",!0),
...
o=o.replace(g,"http://m.qpic.cn"),o=F.addParam(o,"wx_lazy","1",!0);
return B&&o.isGif()&&(window.__addIdKeyReport("28307",106),o=F.addParam(o,"tp","wxpic",!0),
o=B+"hevc?url="+encodeURIComponent(o)+"&type=gif"),"anonymous"==e.crossOrigin&&(o=F.addParam(o,"wx_co","1",!0)),
...
-1!=navigator.userAgent.indexOf("WindowsWechat")?location.href=t:y.invoke("openUrlWithExtraWebview",{
url:t,
openType:1
},function(e){
...
h.isInMiniProgram||y.invoke("profile",{
username:e.username,
...
h.isIOS&&location.href.match(/fontScale=\d+/)&&y.on("menu:setfont",function(e){
...
var dt=e("appmsg/review_image.js"),st=document.getElementById("js_cover"),pt=[];
st&&pt.push(st),new dt({
container:document.getElementById("js_content"),
...
console.log("originalVideoAdNeedData register"),window.originalVideoAdFrames=[],
window.originalVideoAdCurrentFrame=null,window.originalVideoAdFramesAdData=[],window.originalVideoAdFramesUnsetList=[],
...
}),f(),A(),v.tap(document.getElementById("js_hotspot_area"),function(e){
...
-1!=navigator.userAgent.indexOf("MicroMessenger")&&(h.isIOS||h.isAndroid||h.isWp)?y.invoke("openUrlWithExtraWebview",{
url:t,
openType:1
},function(e){
-1==e.err_msg.indexOf("ok")&&(location.href=t);
...
v.on(r,"click",d),v.on(n,"click",d),v.on(document,"click",function(e){
...
var mt=Math.random();
if(2e-4>mt)try{
for(var lt=document.getElementsByTagName("img"),wt=window.screen.height,_t=window.screen.width,gt=0,ut=window.devicePixelRatio,gt="",N=0,ft=lt.length;ft>N;N++){
var ht=lt[N].getAttribute("data-src");
if(ht){
var At=lt[N].getBoundingClientRect();
gt+=_t+"|"+wt+"|"+At.left.toFixed(2)+"|"+(_t-At.right).toFixed(2)+"|"+At.width.toFixed(2)+"|"+ut.toFixed(2)+"|"+ht+";";
}
}
_({
